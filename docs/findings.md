# Findings — CPU vs GPU inference in the browser

The reasoning, the experiments and the measurements behind the numbers in the
[README](../README.md). The README carries the headline result; this document
carries how it was arrived at, including the parts that undercut it.

It is written in the order the work happened, so later sections correct earlier
ones rather than replacing them. Where a figure here disagrees with the
...
- [Finding: the GPU is slower than the CPU](#finding-the-gpu-is-slower-than-the-cpu)
  — why a 0.6 MFLOP model loses on a GPU
- [Second experiment: direct WebGPU](#second-experiment-direct-webgpu) — the same
  model hand-written against the WebGPU API, to price a dispatch
- [Third experiment: a model built into the browser](#third-experiment-a-model-built-into-the-browser)
  — the same model as a Web API in a custom Chromium build, plus every
  re-measurement that followed, up to
  [Chrome's lazy fence](#the-round-trip-that-wasnt-chromes-lazy-fence)
- [The experiment](#the-experiment) — the harness, the traps it had to clear, and
  the limitations of the whole exercise
- [Notes and gotchas](#notes-and-gotchas) — the implementation potholes

Raw data for everything below is in [`measurements/`](measurements/), one file
...
> Current figures are in the [README's results table](../README.md#results).
...
**Every figure above predates that change** and was taken under the
...
[verified to run](measurements/2026-08-24-fixed-count-harness-verification.md),
...
[`docs/measurements/2026-08-23-android-webgpu.json`](measurements/2026-08-23-android-webgpu.json),
...
[`docs/measurements/2026-08-23-custom-chromium-three-way.md`](measurements/2026-08-23-custom-chromium-three-way.md).
...
Full data: [`docs/measurements/2026-08-24-stock-chrome-litert-vs-webgpu.md`](measurements/2026-08-24-stock-chrome-litert-vs-webgpu.md),
raw samples in
[`…-litert-vs-webgpu.json`](measurements/2026-08-24-stock-chrome-litert-vs-webgpu.json).
...
- **It is not the arithmetic, the shader, the language or the buffer.** Enqueue is
  ...
  [coalescing](measurements/2026-08-24-coalesce-w1-ab.md) changed nothing;
...
([data](measurements/2026-08-24-webgpu-two-submit.md)). The full
...
([data](measurements/2026-08-24-webgpu-mapasync-poll.md)). Asking for a
...
Full data:
[`docs/measurements/2026-08-24-stock-chrome-cpu-baseline.md`](measurements/2026-08-24-stock-chrome-cpu-baseline.md),
raw samples in
[`…-cpu-baseline.json`](measurements/2026-08-24-stock-chrome-cpu-baseline.json).
...
twice instead of once per iteration. **The figures in the earlier sections were
taken before this change**, under a 5 ms budget (max 50 runs).
